...
`@nova/core`), now owns four pieces of framework-agnostic logic that
...
import these from `"@nova/core"`. The old `src/utils/copyTemplate.ts` and
...
regular dependency on `@nova/core`, resolved locally via npm/pnpm/yarn
...
(Phase 7) — means every later phase builds directly on `@nova/core` instead
...
- **Establishes the workspace boundary early.** Phase 3's `plugins/next`
  package can depend on `@nova/core` the same way the CLI does, instead of
...
- **One more package to build.** `npm run build` now builds `@nova/core`
  ...
  symlink for `@nova/core` exists) but is a one-time cost.
...
- **Root `tsconfig.json` gained a `paths` mapping** to `packages/core/src`
  so editors/`tsc --noEmit` resolve `@nova/core` correctly even before a
...
- New code in `src/` (or, from Phase 3 onward, in `plugins/*`) should import
  from `"@nova/core"`, not from `src/utils/*`.
...
- Before running `npm run build`, `npm run dev`, or `npm run typecheck` for
  ...
  root so the `@nova/core` workspace package gets linked into
  `node_modules`.
...
`plugins/next`, which will depend on `@nova/core` exactly as the CLI does
today.
